...
from cipher.models import BaseSession, Time, Transaction, Transactions
...
def test_open(transaction_open):
    session = BaseSession(transactions=Transactions([transaction_open]))
...
def test_closed(transaction_open, transaction_close):
    session = BaseSession(
        transactions=Transactions([transaction_open, transaction_close])
    )
